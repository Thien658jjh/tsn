let config = require('../config/apiConfig').CONFIG_API,
    ...
    __verify_keyupload:(keyupload) => {
        if(keyupload === config.__KEY_UPLOAD){
            return true
        } else {
            return false
        }
    ...
    __verify_mac_address:(macAddress) => {
        if (macAddress === config.__MY_MAC_ID1 || macAddress === config.__MY_MAC_ID2 || macAddress === config.__MY_MAC_ID3) {
            return true
        }
        else {
            return false
    ...
    __check_header_request: (req) => {
        if (req['apikey'] === config.__API_KEY && req['api-secret'] === config.__API_SECRET &&  req['api-author'] === config.__API_AUTHOR) {
            return true
        }else {
            return false
        }
